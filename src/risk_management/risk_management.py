...
class RiskManager:
    ...
    def calculate_adaptive_position_size(
        ...
        risk_capital = self.portfolio_value * self.parameters.max_risk_per_trade
        # Use inverse volatility scaling
        if volatility > 0:
            size = risk_capital / (volatility * price)
        else:
            size = risk_capital / price
        # Adjust by confidence
        size *= confidence
        # Cap by max position size
        max_size = self.portfolio_value * self.parameters.max_position_size / price
        return min(size, max_size)

    def adjust_risk_parameters(self, recent_volatility: float) -> None:
        """Dynamically adjust risk constraints based on market volatility"""
        if recent_volatility > self.parameters.volatility_threshold_high:
            # High volatility: tighten constraints
            self.parameters.max_risk_per_trade *= 0.5
            self.parameters.max_position_size *= 0.5
        elif recent_volatility < self.parameters.volatility_threshold_low:
            # Low volatility: loosen constraints
            self.parameters.max_risk_per_trade = min(
                self.parameters.max_risk_per_trade * 1.2,
                self.parameters.max_risk_per_trade_default
            )
            self.parameters.max_position_size = min(
                self.parameters.max_position_size * 1.2,
                self.parameters.max_position_size_default
            )
