...
class AITradingEngine:
    ...
    def __init__(self, ai_provider: str = "openai", model: str = "gpt-4", use_ensemble: bool = False):
...
async def cached_openai_call(prompt: str, model: str, ai_provider: str):
    cache_key = (ai_provider, model, prompt)
    ...
    response = await openai.ChatCompletion.acreate(
        model=model,
        messages=[
            {"role": "system", "content": "You are an expert financial advisor and trading strategist."},
    ...
    async def _analyze_with_primary_model(self, prompt: str) -> Dict[str, Any]:
        ...
        if self.ai_provider == "openai":
            response = await cached_openai_call(prompt, self.model, self.ai_provider)
...
class AITradingStrategy(AIStrategy):
    ...
    def __init__(self, ai_provider: str = "openai", model: str = "gpt-4", use_ensemble: bool = False):
        super().__init__(ai_provider, {"model": model, "use_ensemble": use_ensemble})
        self.engine = AITradingEngine(ai_provider, model, use_ensemble)
